...
[[entity-edit-process-form-example-1]]
===== Entity Edit Process Form Example 1
...
<4> When the _Complete Task_ button is clicked, the changes of the _Order_ entity are saved to the database and then the process task is completed

[[entity-edit-process-form-example-2]]
===== Entity Edit Process Form Example 2

In the <<entity-edit-process-form-example-1>> we stored the entity identifier _orderId_ in process variables. We also can store an entity itself. We can start the process this way:
...
In the previous example we had to manually find the order entity using the id, then merge it to the data context and set to the data container. In case of having an entity among process variables, the process form  may be as follows. The XML descriptor is the same as in the previous example:

.order-edit-task-form-2.xml
[source, xml]
----
<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<window xmlns="http://schemas.haulmont.com/cuba/screen/window.xsd"
...
.OrderEditTaskForm2.java
[source, java]
----
@ProcessForm
@UiController("ref_OrderEditTaskForm2")
@UiDescriptor("order-edit-task-form-2.xml")
public class OrderEditTaskForm2 extends Screen implements ProcessFormContextAware {
    ...
    @ProcessVariable <1>
    @BindToDataContainer(container = "orderDc") <2>
    private Order order;
...
----
<1> We inject the _order_ variable. When we get a process variable of the _entity_ type it is reloaded with the _local view.
<2> If the process variable is annotated with the `@BindToDataContainer`, then the entity is reloaded with the view of the corresponding data container, then the entity is merged to the data context and is set to the data container.