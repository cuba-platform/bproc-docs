...
public class ParticipantSelectionForm extends Screen implements ProcessFormContextAware {
    ...
    private void onBeforeShow(BeforeShowEvent event) {
        Map<String, String> formParams = processFormContext.getFormParams(); <2>
...
<2> `getFormParams()` method of the `ProcessFormContext` returns a map of parameters defined for this form in the modeler.
...
public class DynamicOutcomesForm extends Screen implements ProcessFormContextAware {
    ...
    private void initOutcomesPanel() {
        ProcessFormData formData = processFormContext.getFormData(); <1>
        formData.getOutcomes().forEach(this::createOutcomeBtn); <2>
    }

    private void createOutcomeBtn(ProcessFormOutcome outcome) {
...
----
<1> The `ProcessFormData` can be gotten from the _processFormContext_
