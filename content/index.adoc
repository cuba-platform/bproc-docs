...
"Form Field" Assignee Source::

If you select the _Form field_ assignee source then a _LookupField_ component will be displayed in the element properties panel. This component will contain all dynamic fields of type _User_ from other model elements. For example, if you declared a field with id _manager_ of type _User_ for the start event node, then the _manager_ field will be displayed in the lookup for all user tasks in the model. Obviously, the user task will be assigned to the user that was selected on the start form when the process was started.

"Process Variable" Assignee Source::

The _Process variable_ assignee source is similar to the _Form field_ source, but the dropdown list of possible values contains process variables declared in the _Process variables_ block of the process properties panel. A process may be started programmatically without using of the start process form. Process variables values will be passed from the API. In this case, if you need to assign a user task to the person from one of process variables, you can either write an explicit expression for the assignee or add an info about the process variable that can be used in the model to the _Process variables_ section of the process properties panel.
